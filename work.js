...
bot.hear(/^старт$/i, async msg => {
    ...
    msg.send(`Накрутка рабов запущена!
      Разработчик бота: @korn9kov`)
});
    ...
    res.slaves.map(async user => {
        if(user.fetter_to === 0){
            await fetch(`https://pixel.w84.vkforms.ru/HappySanta/slaves/1.0.0/buyFetter`,{
                ...
                vk.api.messages.send({peer_id: 2000000000 + config.vk.chat_id, random_id: utils.random(2000, 2020202020), message: `[БОТ]: Купил оковы для @id${user.id}`})
        }
            ...
            vk.api.messages.send({peer_id: 2000000000 + config.vk.chat_id, random_id: utils.random(2000, 2020202020), message: `[БОТ]: Установил работу для @id${user.id}`})
        }
    })
});
};
...
setInterval(buyFetter, config.intervals.buyFetter * 1000);
